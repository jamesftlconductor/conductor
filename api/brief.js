...
- Do NOT pair a relative-count phrase ("in five days", "five days from now", "in a few days", "next week") with the day-and-date for the same signal. These counts are computed by you and frequently disagree with the date — and the day-and-date is already enough. Pick one: either use the lifted day-and-date phrase ("Wednesday, May 20") OR use "tomorrow"/"today" (the only relative phrases safe to compute). Never both. Exception: "today" and "tomorrow" are safe because the ETA's friendly field will already say so when applicable.`;
